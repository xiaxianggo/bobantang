...
- (instancetype)initWithUpperRightFrame:(CGRect)frame;
@end
...
- (void)showWithContentView:(UIView *)contentView andAnimated:(BOOL)animated {
    ...
    KGModalCloseButton *closeButton = [[KGModalCloseButton alloc] initWithUpperRightFrame:(CGRect){containerViewRect.size.width-42,11,32,32}];
...
- (void)drawRect:(CGRect)rect{
    ...
    if([[KGModal sharedInstance] backgroundDisplayStyle] == KGModalBackgroundDisplayStyleSolid){
        [[UIColor colorWithWhite:0.1 alpha:0.1] set];
        CGContextFillRect(context, self.bounds);
    }else{
        CGContextSaveGState(context);
        size_t gradLocationsNum = 2;
        CGFloat gradLocations[2] = {0.0f, 1.0f};
        CGFloat gradColors[8] = {0, 0, 0, 0.3, 0, 0, 0, 0.2};
...
- (UIImage *)closeButtonImage{
    ...
    //// Color Declarations
    UIColor *topGradient = [UIColor colorWithRed:1.0 green:0.55 blue:0.00 alpha:0.76];
    UIColor *bottomGradient = [UIColor colorWithRed:1.0 green:0.55 blue:0.00 alpha:0.76];
